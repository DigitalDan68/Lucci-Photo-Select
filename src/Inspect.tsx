...
export default function Inspect({photo,zoom,zoomRevision=0}:{photo:Photo;zoom:boolean;zoomRevision?:number}){
 ...
 return <section className="inspect-pane" data-photo-id={photo.id}><div className="inspect-label"><b>{photo.stem}</b><span>{data?.kind||'Loading full image…'}</span></div>{error&&<p role="alert">{error}</p>}{data?.warning&&<p>{data.warning}</p>}
