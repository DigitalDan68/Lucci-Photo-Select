...
app.whenReady().then(async()=>{
  ...
  protocol.handle('photo',async req=>{try{let file=photoPath(req.url);const photos=catalog.library?.photos||[];const item=photos.find(p=>[p.previewUrl,p.fullPreviewUrl].includes(req.url)||p.decodedPath===file)
...
ipcMain.handle('photo:inspect',(_e,id:string)=>catalog.inspect(id))
