...
export function assessQuality(focus:number,exposure:number,noise:number,hasReliableEyes:boolean){
 ...
 // Missing face/eye detection means uncertain subject location, not a poor photograph.
 // Keep it below a confident five-star technical score, but do not force it below "good".
 const score=Math.min(combined,1+4*f,hasReliableEyes?5:4.25)
