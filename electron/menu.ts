...
export function menuTemplate(platform:string,send:(action:string)=>void):MenuItemConstructorOptions[]{
 ...
 {label:'Edit',submenu:[item('Undo','undo','CmdOrCtrl+Z'),item('Redo','redo','CmdOrCtrl+Shift+Z'),{type:'separator'},{role:'cut'},{role:'copy'},{role:'paste'},item('Select All Photos','select','CmdOrCtrl+A'),item('Deselect All Photos','deselect','CmdOrCtrl+Shift+A')]},
