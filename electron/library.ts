...
export class Catalog {
  ...
  async load(){try{this.library=JSON.parse(await fs.readFile(path.join(this.root,'library.json'),'utf8'));this.normalize()}catch{}return this.library}
  normalize(){for(const p of this.library?.photos||[]){p.previewUrl=photoUrl(photoPath(p.previewUrl));if(p.fullPreviewUrl)p.fullPreviewUrl=photoUrl(photoPath(p.fullPreviewUrl))}}
  ...
  async change(ids:string[],patch:Patch){if(this.busy)throw new Error('Wait for the current operation');if(!this.library)return null;this.remember();const set=new Set(ids);for(const p of this.library.photos){if(!set.has(p.id))continue;if(patch.stars!==undefined)p.stars=Math.max(0,Math.min(5,Math.round(patch.stars)));if(patch.note!==undefined)p.note=String(patch.note).slice(0,20000);if(patch.bookmarked!==undefined)p.bookmarked=!!patch.bookmarked;if(patch.flag&&['none','pick','reject'].includes(patch.flag))p.flag=patch.flag}await this.persist();return this.library}
  ...
  async importFiles(files:string[]){const signal=this.start(),r=report('Import'),cache=path.join(this.root,'cache',randomUUID()),photos:Photo[]=[]
    try{const groups=new Map<string,{stem:string;jpeg?:string;raw?:string}>()
      ...
      let bytes=0;for(const g of groups.values())for(const f of[g.jpeg,g.raw])if(f)bytes+=(await fs.stat(f)).size;await space(cache,bytes*1.2)
      ...
      for(const g of groups.values()){if(signal.aborted)break;index++;this.progress({phase:'Importing and ranking',current:index,total:groups.size,file:g.stem})
        const dir=path.join(cache,randomUUID());try{const hash=await digest(g.raw||g.jpeg!,signal);if(seen.has(hash)){r.skipped++;continue}seen.add(hash);await fs.mkdir(dir,{recursive:true})
          const jpeg=g.jpeg?path.join(dir,path.basename(g.jpeg)):undefined,raw=g.raw?path.join(dir,path.basename(g.raw)):undefined
          if(jpeg)await safeCopy(g.jpeg!,jpeg,signal);if(raw)await safeCopy(g.raw!,raw,signal)
          if(await digest(raw||jpeg!,signal)!==hash)throw new Error('Imported original failed checksum verification')
          ...
          if(raw){try{const tags=await exifr.parse(raw,{pick:['DateTimeOriginal','Model','ISO','LensModel']});if(tags){metrics.capturedAt=tags.DateTimeOriginal instanceof Date?tags.DateTimeOriginal.toISOString():metrics.capturedAt;metrics.camera=tags.Model||metrics.camera;metrics.lens=tags.LensModel||metrics.lens;metrics.iso=tags.ISO||metrics.iso}}catch{}}
          photos.push({id:randomUUID(),stem:g.stem,jpegPath:jpeg,rawPath:raw,sourceJpeg:g.jpeg,sourceRaw:g.raw,hash,previewUrl:photoUrl(preview),fullPreviewUrl:photoUrl(analysis!),bookmarked:false,flag:'none',note:'',importedAt:new Date().toISOString(),...metrics});r.completed++
  ...
  async inspect(id:string):Promise<Inspection>{const p=this.library?.photos.find(p=>p.id===id);if(!p)throw new Error('Photo not found');const raw=this.resolve(p,'raw');let warning:string|undefined
    if(raw){const out=path.join(this.root,'decoded',p.id+'.png');await fs.mkdir(path.dirname(out),{recursive:true});try{if(!existsSync(out))await imageJob('decode',raw,out);const m=await sharp(out).metadata();p.decodedPath=out;return{url:photoUrl(out),width:m.width!,height:m.height!,kind:'RAW decoded'}}catch(e){warning=`RAW decode failed: ${(e as Error).message}. Showing available preview.`}}
  ...
  async saveProject(file:string){const signal=this.start(),r=report('Save portable project');const assets=path.join(path.dirname(file),path.basename(file)+'.assets-'+randomUUID());try{if(!this.library)throw new Error('No library');const copy=structuredClone(this.library);let bytes=0;for(const p of copy.photos)for(const kind of['raw','jpeg'] as const){const f=this.resolve(p,kind);if(f)bytes+=(await fs.stat(f)).size}await space(assets,bytes*1.2)
    for(let i=0;i<copy.photos.length;i++){if(signal.aborted)throw new Error('Cancelled');const p=copy.photos[i];this.progress({phase:'Saving portable project',current:i+1,total:copy.photos.length,file:p.stem});const dir=path.join(assets,p.id);await fs.mkdir(dir,{recursive:true})
      for(const [key,kind] of [['rawPath','raw'],['jpegPath','jpeg']] as const){if(!p[key])continue;const src=this.resolve(p,kind);if(!src)throw new Error(`${p.stem}: original missing. Locate files before saving.`);const dst=path.join(dir,path.basename(src));await safeCopy(src,dst,signal);p[key]=path.relative(path.dirname(file),dst)}
      for(const key of['previewUrl','fullPreviewUrl'] as const){if(!p[key])continue;let src=photoPath(p[key]!);if(!existsSync(src)){const original=this.library.photos.find(x=>x.id===p.id)!;src=this.resolve(original,'jpeg')||''}if(!src){if(key==='previewUrl')throw new Error(`${p.stem}: preview missing; locate files before saving`);delete p.fullPreviewUrl;continue}const dst=path.join(dir,key+path.extname(src));await safeCopy(src,dst,signal);p[key]=path.relative(path.dirname(file),dst)}delete p.decodedPath;r.completed++}
    delete copy.cacheDir;delete copy.projectPath;await atomic(file,{format:'LPV',version:2,library:copy});this.library.projectPath=file;await this.persist();r.folder=path.dirname(file);return r
    }catch(e){await fs.rm(assets,{recursive:true,force:true});throw e}finally{this.controller=undefined}}
  async openProject(file:string){if(this.busy)throw new Error('Another operation is running');const doc=JSON.parse(await fs.readFile(file,'utf8'));if(doc.format!=='LPV'||![1,2].includes(doc.version)||!Array.isArray(doc.library?.photos))throw new Error('Invalid LPV project');const lib=doc.library as Library
    ...
    lib.projectPath=file;delete lib.cacheDir;this.library=lib;this.normalize();this.history=[];this.future=[];await this.persist();return lib}
