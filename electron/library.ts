...
export class Catalog {
  ...
  async newProject(file:string){if(this.busy)throw new Error('Wait for the current operation');const name=path.basename(file,path.extname(file));const library:Library={name,source:path.dirname(file),createdAt:new Date().toISOString(),projectPath:file,photos:[]};const saved=structuredClone(library);delete saved.projectPath;await atomic(file,{format:'LPV',version:3,library:saved});this.library=library;this.history=[];this.future=[];await this.persist();return library}
